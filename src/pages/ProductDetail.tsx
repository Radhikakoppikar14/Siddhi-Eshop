...
export const ProductDetail: React.FC = () => {
  ...
  return (
    <div className="py-6 sm:py-8 bg-zinc-50 min-h-screen overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        
        {/* Breadcrumb Navigation */}
        <nav className="flex items-center gap-1.5 sm:gap-2 text-xs text-zinc-500 mb-6 font-mono flex-wrap">
        ...
        {/* Main PDP Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 bg-white rounded-2xl sm:rounded-3xl border border-zinc-200 p-4 sm:p-10 shadow-2xs mb-10 overflow-hidden">
          
          {/* Left Column: Image Stage & Gallery */}
          <div className="lg:col-span-6 space-y-4">
            <div className="h-72 sm:h-96 w-full rounded-2xl bg-zinc-50 border border-zinc-100 p-4 sm:p-6 flex items-center justify-center relative overflow-hidden">
              <img
                src={productImage}
                alt={productName}
              ...
              />
              <div className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-white/90 backdrop-blur-xs border border-zinc-200 px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wider text-zinc-700 uppercase font-mono">
                {brandName}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3 text-center text-xs">
              <div className="p-2 sm:p-3 bg-zinc-50 border border-zinc-200 rounded-xl">
                <ShieldCheck size={18} className="text-emerald-500 mx-auto mb-1" />
                <span className="font-bold text-zinc-900 block text-[10px] sm:text-[11px]">100% Genuine</span>
                <span className="text-[9px] sm:text-[10px] text-zinc-500">OEM Certificate</span>
              </div>
              <div className="p-2 sm:p-3 bg-zinc-50 border border-zinc-200 rounded-xl">
                <Clock size={18} className="text-zinc-700 mx-auto mb-1" />
                <span className="font-bold text-zinc-900 block text-[10px] sm:text-[11px]">Bangalore Stock</span>
                <span className="text-[9px] sm:text-[10px] text-zinc-500">&lt;24h Dispatch</span>
              </div>
              <div className="p-2 sm:p-3 bg-zinc-50 border border-zinc-200 rounded-xl">
                <FileText size={18} className="text-zinc-700 mx-auto mb-1" />
                <span className="font-bold text-zinc-900 block text-[10px] sm:text-[11px]">GST Invoicing</span>
                <span className="text-[9px] sm:text-[10px] text-zinc-500">18% Input Credit</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col justify-between space-y-6">
            <div>
              ...
              <div className="flex flex-wrap items-center justify-between text-xs font-semibold mb-2 gap-2">
              ...
              {/* Title */}
              <h1 className="text-xl sm:text-3xl font-black text-zinc-950 leading-tight">
                {productName}
              </h1>
              ...
              {/* Price Banner */}
              <div className="mt-5 p-3.5 sm:p-4 bg-zinc-50 border border-zinc-200 rounded-2xl flex items-baseline justify-between gap-2">
                <div>
                  <span className="text-[10px] sm:text-[11px] text-zinc-400 font-semibold block uppercase tracking-wider font-mono">
                    Basic Unit Rate (Excl. 18% GST)
                  </span>
                  <div className="text-xl sm:text-2xl font-black text-zinc-950 font-mono tabular-nums mt-0.5">
                ...
                <div className="text-right">
                  <span className="text-[10px] sm:text-[11px] text-zinc-400 block font-mono">Est. with GST:</span>
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 text-xs font-mono">
                      {["2 Core", "3 Core", "4 Core", "5 Core", "7 Core", "12 Core"].map((c) => (
                        <button
                          ...
                          className={`py-1.5 px-1 rounded-lg border font-semibold transition-all text-center ${
                            selectedCore === c
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-xs text-zinc-600">
                    {catalogProduct.specs.map((sp, idx) => (
                      <li key={idx} className="flex items-start gap-2">
                        ...
                        <span className="break-words">{sp}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="pt-6 border-t border-zinc-200 space-y-3">
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <div className="flex items-center justify-between sm:justify-start border border-zinc-200 rounded-xl bg-zinc-50 p-1">
                  ...
                  <input
                    ...
                    className="w-20 sm:w-16 text-center text-xs font-bold font-mono bg-white border border-zinc-200 rounded-lg py-1.5 outline-none"
                  />
                </div>

                <button
                  onClick={handleAddToCart}
                  className="flex-1 py-3 px-4 bg-zinc-950 hover:bg-zinc-800 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer"
                >
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 px-3 bg-zinc-100 hover:bg-zinc-200 text-zinc-900 font-semibold text-xs rounded-xl transition-colors flex items-center justify-center gap-2 border border-zinc-200 cursor-pointer text-center"
                >
                  <FileText size={14} className="shrink-0" />
                  <span className="truncate">Request Commercial Proforma</span>
                </button>

                <Link
                  to="/olflex-cables"
                  className="px-4 py-2.5 bg-zinc-50 hover:bg-zinc-100 text-zinc-700 font-semibold text-xs rounded-xl transition-colors shrink-0 border border-zinc-200 flex items-center justify-center gap-1.5"
                >
                  <span>Catalog</span>
                  <ArrowRight size={13} />
                </Link>
